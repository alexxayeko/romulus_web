...
================================================== */
if ( ! function_exists( 'inusti_base_sidebar' ) ) {
     function inusti_base_sidebar() {
        // VARIABLES
        global $post;

        // DEFAULT SIDEBAR CONFIG
        $default_sidebar_config = inusti_get_option('default_sidebar_config', ''); 
        $default_left_sidebar   = inusti_get_option('default_left_sidebar', ''); 
        $default_right_sidebar  = inusti_get_option('default_right_sidebar', '');
        ...
        if ( is_search() || is_archive() || is_author() || is_category() || is_home() ) {
          $default_sidebar_config = inusti_get_option('archive_sidebar_config', ''); 
          $default_left_sidebar   = inusti_get_option('archive_sidebar_left', '');
          $default_right_sidebar  = inusti_get_option('archive_sidebar_right', ''); 
        }
        ...
        if ( $post && is_singular() ) {
          $sidebar_config = get_post_meta( $post->ID, 'inusti_sidebar_config', true );
          $left_sidebar   = get_post_meta( $post->ID, 'inusti_left_sidebar', true );
          $right_sidebar  = get_post_meta( $post->ID, 'inusti_right_sidebar', true );
        }
...
function inusti_sidebar_global($sidebar_layout_config, $sidebar_left_config, $sidebar_right_config){
...
function inusti_base_layout($template){
  $post_id = inusti_id();
  $sidebar_var           = inusti_base_sidebar();
  ...
  $sidebar_config = inusti_sidebar_global($sidebar_var['config'], $sidebar_var['left'], $sidebar_var['right']);
  extract($sidebar_config);
  
  $page_full_width = get_post_meta($post_id, 'inusti_page_full_width', true);
 
  ?>

  <div id="wp-main-content" class="clearfix main-page">
  <?php do_action( 'inusti_before_page_content' ); ?>
      ...
      <?php do_action( 'inusti_before_main_content' ); ?>
       <div class="main-page-content base-layout row <?php echo esc_attr($page_wrap_class); ?>">
            ...
            <div class="sidebar wp-sidebar sidebar-left <?php echo esc_attr($left_sidebar_config['class']); ?>">
               <?php do_action( 'inusti_before_sidebar' ); ?>
               <div class="sidebar-inner">
                  <?php dynamic_sidebar($left_sidebar_config['name'] ); ?>
               </div>
               <?php do_action( 'inusti_after_sidebar' ); ?>
            </div>
            <?php endif ?>
        ...
        <?php do_action( 'inusti_after_main_content' ); ?>
      </div>
    </div>
    <?php do_action( 'inusti_after_page_content' ); ?> 
  </div>    
     <?php
}

if ( ! function_exists( 'inusti_get_template' ) ) {
    function inusti_get_template( $template ) {
