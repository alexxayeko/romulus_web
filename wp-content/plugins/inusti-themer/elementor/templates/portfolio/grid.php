<?php
  ...
  $this->add_render_attribute('wrapper', 'class', ['gva-portfolio-grid clearfix', 'grid-' . $_random, 'text-' . $settings['align']]);
      ...
      ?>
       <nav class="portfolio-filter">
          <ul class="nav nav-tabs">
             <li><a class="btn-filter all active" href="javascript:void(0)" data-filter="*"><span><?php echo esc_html__( 'All', 'inusti-themer' ); ?></span><span class="count"></span></a></li>
             <?php 
             if ( !empty($terms) && !is_wp_error($terms) ){ 
                foreach ( $terms as $term ) {
             ?>
               <li><a href="javascript:void(0)" class="btn-filter" data-filter=".<?php echo esc_attr($term->slug) ?>"><span><?php echo esc_html($term->name) ?></span><span class="count"></span></a></li>
             <?php 
                }
             }
             ?>
